...
def build_df(json_path: str, threshold: int, preprocessed_csv: str = None):     
    ...
    # First condition to kill leafs: leaf is under threshold
    condition_leafs = df["leaf"].isin(leafs_to_kill)
    # Loops the columns to replace leafs not unde threshold with nan
    for i in reversed(range(number_of_categories_columns)):
        # Second condition to kill leafs: makes sure that only removes the category if it's a leaf
        condition_depth = df["max_depth"] == i
    ...
    # Has a custom dictionary for products labeled as "other"
    ...
    # Change "other" for np.nan in category_0 columns
    df_nan = df.copy()
...
def build_base_dataframe(json_path, preprocessed_csv=None):
    ...
    for dic in products_dic:
        ...
        if preprocessed_csv == None:
            ...
            new_dict["image"] = dic["image"]
            new_dict["url"] = dic["url"]
